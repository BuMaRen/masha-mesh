...
func aggregate(ctx context.Context, engine *gin.Engine) {
	// 这里可以注册多个 webhook handler，分别处理不同的资源事件
	// 例如：
	// engine.POST("/webhook/container", containerWebhookHandler)
	// engine.POST("/webhook/another-resource", anotherResourceWebhookHandler)
}
